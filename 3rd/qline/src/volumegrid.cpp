...
VolumeGrid::VolumeGrid(QWidget* parent) : AutoGrid( parent)
{
...
void VolumeGrid::paintEvent(QPaintEvent *event)
{
...
bool VolumeGrid::readData(QString strFile)
{
...
void VolumeGrid::update_stat(const struct figure_stat &stat)
{

}

//获得画图所需的各项指标
void VolumeGrid::getIndicator()
{
    maxVolume = 0;
    if (mDataFile.kline.size() <= 0) {
        return;
...
void VolumeGrid::drawAverageLine(int day)
{
    //画线要连接的点

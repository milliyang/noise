...
LineGrid::LineGrid(QWidget *parent) : AutoGrid(parent)
{
    //开启鼠标追踪
...
bool LineGrid::readData(QString strFile)
{
...
void LineGrid::paintEvent(QPaintEvent *event)
{
    AutoGrid::paintEvent(event);

    if (mDataFile.kline.size() <= 0) {
        return;
    }
...
void LineGrid::resizeEvent(QResizeEvent* event)
{
...
void LineGrid::drawCrossVerLine()
{
    QPainter painter(this);
    QPen     pen;
...
void LineGrid::drawCrossHorLine()
{
    QPainter painter(this);
    QPen     pen;
    pen.setColor(QColor("#FFFFFF"));
...
void LineGrid::drawMouseMoveCrossVerLine()
{
...
void LineGrid::drawMouseMoveCrossHorLine()
{
...
void LineGrid::drawAverageLine(int day)
{
...
void LineGrid::update_stat(const struct figure_stat &stat)
{
